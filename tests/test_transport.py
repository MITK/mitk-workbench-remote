...
def test_custom_timeout() -> None:
    t = RestTransport(BASE, timeout=5.0)
...
def test_get_binary_sets_transfer_mode_header() -> None:
    ...
    assert responses.calls[0].request.headers.get("X-MITK-Transfer-Mode") == TransferMode.DIRECT
    t.close()
...
def test_get_binary_transfer_mode_header_can_be_overridden() -> None:
    responses.add(responses.GET, _api("/nodes/abc/data"), body=b"data", status=200)
    t = RestTransport(BASE, transfer_mode=TransferMode.DIRECT)
    t.get_binary("/nodes/abc/data", headers={"X-MITK-Transfer-Mode": TransferMode.FILE_REFERENCE})
    assert (
...
def test_503_raises_DataStorageNotAvailableError() -> None:
    ...
    t = RestTransport(BASE)
    with pytest.raises(errors.DataStorageNotAvailableError):
        t.get("/nodes")
    t.close()
...
def test_connection_error_raises_ConnectionError() -> None:
    ...
    t = RestTransport(BASE)
    with pytest.raises(errors.MitkConnectionError):
        t.get("/nodes")
    t.close()
...
def test_transfer_mode_file_reference_when_localhost_and_supported() -> None:
    ...
    t = RestTransport(BASE)  # 127.0.0.1 → localhost
...
def test_transfer_mode_remote_host() -> None:
    # No server stub registered — if a request were made it would raise
    t = RestTransport(BASE_REMOTE)
    assert t.transfer_mode == TransferMode.DIRECT
    assert len(responses.calls) == 0
    t.close()
